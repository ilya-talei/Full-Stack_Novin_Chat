import User from "../../database/models/user.js";
import { Op, UniqueConstraintError } from "sequelize";
import bcrypt from "bcrypt";
import { AppError } from "../../middlewares/errorMiddleware.js";
import type { createValidationType, updateValidationType } from "./controller.js";

class UserService {
    static async index(page: number, limit: number, search: string) {
        const offset: number = (page - 1) * limit;
        const searchNumber: number = parseInt(search);
        const users = await User.findAndCountAll({
            where: {
                [Op.or]: [
                    {
                        login_id: {
                            [Op.like]: `%${search}%`,
                        },
                    },
                    {
                        phone: {
                            [Op.like]: `%${search}%`,
                        },
                    },
                    {
                        employee_id: isNaN(searchNumber) ? -1 : searchNumber,
                    },
                ],
            },
            offset: offset,
            limit: limit,
            order: [["created_at", "DESC"]],
            attributes: {
                exclude: ["hashed_password"],
            },
        });
        return users;
    }

    static async show(id: number) {
        const user = await User.findByPk(id, {
            attributes: {
                exclude: ["hashed_password"],
            },
        });
        return user;
    }

    static async create(userData: createValidationType) {
        const data = {
            login_id: userData.login_id,
            phone: userData.phone,
            employee_id: userData.employee_id,
            hashed_password: await bcrypt.hash(userData.password, 10),
            active: userData.active ?? true,
        };
        let user;
        try {
            user = await User.create(data);
        } catch (error: unknown) {
            if (error instanceof UniqueConstraintError) {
                const field = error.errors[0]?.path;

                const messages = {
                    login_id: "شناسه ورود تکراری است",
                    phone: "شماره تلفن تکراری است",
                    employee_id: "شناسه کارمندی تکراری است",
                };

                throw new AppError(
                    field !== null && field !== undefined
                        ? (messages[field as keyof typeof messages] ?? "مقدار تکراری است")
                        : "مقدار تکراری است",
                    400,
                );
            } else {
                throw error;
            }
        }
        const u = user.get({ plain: true });
        const { id, login_id, phone, employee_id, active, created_at, updated_at } = u;
        return { id, login_id, phone, employee_id, active, created_at, updated_at };
    }

    static async update(id: number, userData: updateValidationType) {
        const user = await User.findByPk(id);
        if (!user) {
            throw new AppError("User not found", 404);
        }

        const hashed_password =
            userData.password !== undefined
                ? await bcrypt.hash(userData.password, 10)
                : user.hashed_password;

        try {
            await user.update({
                login_id: userData.login_id ?? user.login_id,
                phone: userData.phone ?? user.phone,
                employee_id: userData.employee_id ?? user.employee_id,
                hashed_password: hashed_password,
                active: userData.active ?? user.active,
            });
        } catch (error: unknown) {
            if (error instanceof UniqueConstraintError) {
                const field = error.errors[0]?.path;

                const messages = {
                    login_id: "شناسه ورود تکراری است",
                    phone: "شماره تلفن تکراری است",
                    employee_id: "شناسه کارمندی تکراری است",
                };

                throw new AppError(
                    field !== undefined && field !== null
                        ? (messages[field as keyof typeof messages] ?? "مقدار تکراری است")
                        : "مقدار تکراری است",
                    400,
                );
            } else {
                throw error;
            }
        }
        await user.reload();
        const u = user.get({ plain: true });
        const { id: uid, login_id, phone, employee_id, active, created_at, updated_at } = u;
        return { id: uid, login_id, phone, employee_id, active, created_at, updated_at };
    }

    static async delete(id: number) {
        const user = await User.findByPk(id);
        if (!user) {
            throw new AppError("User not found", 404);
        }
        await user.destroy();
        return;
    }

    static async changePassword(
        userId: number,
        currentPassword: string,
        newPassword: string,
    ): Promise<void> {
        const user: User | null = await User.findByPk(userId);
        if (!user) {
            throw new AppError("کاربر پیدا نشد", 404);
        }

        const isMatch: boolean = await bcrypt.compare(currentPassword, user.hashed_password);
        if (!isMatch) {
            throw new AppError("رمز عبور فعلی اشتباه است", 400);
        }

        const hashed_password: string = await bcrypt.hash(newPassword, 10);

        await user.update({ hashed_password: hashed_password });
    }
}

export default UserService;
