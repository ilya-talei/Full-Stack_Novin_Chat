...
پروکسی Vite روی گوشی وجود ندارد. آدرس سرور در فایل `.env.apk` تنظیم می‌شود
(بیلد APK با `vite --mode apk`؛ دسکتاپ با `npm run dev` دست‌نخورده می‌ماند):

```env
VITE_API_BASE_URL=http://192.168.1.4:3001
VITE_WS_URL=http://192.168.1.4:3001
```

IP ویندوز/وای‌فای خود را جایگزین کنید (`ipconfig` → IPv4). گوشی و سیستم باید روی یک شبکه باشند، بک‌اند روی پورت `3001` روشن باشد، و در فایروال ویندوز پورت `3001` برای Inbound باز باشد.

اگر IP عوض شد، `.env.apk` را آپدیت کنید و دوباره `npm run build:apk` بزنید؛ فقط نصب مجدد APK کافی نیست مگر آدرس داخل بیلد درست باشد.
